...
class CRUDBankSampah extends Controller
{
    ...
    return view('DashboardBankSampah', compact('data', 'warnings'));
}

    public function tambah()//
    {
        $data = Sampah::select('IDSampah', 'nama')
        ->get();
    return view('inputsampah', compact('data'));
    }
    ...
    public function update(Request $request, $IDTransaksiSampah)//
    {
    $transaction = TransaksiBank::findOrFail($IDTransaksiSampah);
    $transaction->update([
        'NIK' => $request->NIK,
        'IDBankSampah' => $request->IDBankSampah,
        'IDSampah' => $request->IDSampah,
    ...
    return redirect()->route('main')->with('success', 'Transaction updated!');
}

public function delete($Tanggal, $IDBankSampah)//
{
        $data = TransaksiBank::where('Tanggal', $Tanggal)->where("IDBankSampah", $IDBankSampah)->delete();
